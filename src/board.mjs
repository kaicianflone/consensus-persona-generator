...
async function ensureIndex(boardId, statePath) {
  ...
  const idx = { latest_by_type: {}, persona_by_id: {}, refs: {} };
  for (const j of jobs) {
...
export async function writeArtifact(boardId, { type, payload }, statePath) {
  ...
  const artifacts = { type, payload, created_at: new Date().toISOString() };
  const sub = await engine.submitJob('orchestrator@local', job.id, { summary: `${type} artifact`, artifacts, confidence: 1 });
  ...
  return { success: true, artifact_ref: sub.id, job_id: job.id };
}
